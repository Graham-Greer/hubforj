...
import { initialEventsPageSettingsState } from "../../form-state";
import { updateEventsPageSettingsAction } from "../../actions";
import styles from "../../settings.module.css";

const fieldKeys = Object.keys(initialEventsPageSettingsState.values);

function createSavedValuesSnapshot(values) {
  return createSavedSnapshotFromKeys(
    fieldKeys,
    initialEventsPageSettingsState.values,
    values
  );
}
...
export default function EventsPageSettingsForm({
  hub,
  initialValues,
  mediaAssets = [],
  mediaFolders = [],
  ...
  const initialState = {
    ...initialEventsPageSettingsState,
    values: {
      ...initialEventsPageSettingsState.values,
      ...initialValues,
    },
  ...
  const [state, formAction] = useActionState(updateEventsPageSettingsAction, initialState);
  ...
  const submitIdleLabel =
    state?.success && !isDirty ? "Events page saved" : "Save events page";
    ...
    >
      ...
      <AdminFormSection title="Hero">
        <PageHeroFieldGroup
          ...
          eyebrowHint="Optional short label above the events hero title."
          titleHint="Required main public-facing title for the events route hero."
          descriptionHint="Optional supporting sentence beneath the events hero title. Leave blank to use the system default."
      ...
      <AdminFormFooter ref={feedbackRef} error={state?.error} success={state?.success}>
        ...
        <SubmitButton
          ...
          pendingLabel="Saving events page"
          disabled={!isDirty}
        />
      </AdminFormFooter>
    </form>
