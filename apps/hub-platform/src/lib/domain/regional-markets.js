...
const launchFormattingLocales = ["en-GB", "en-US", "en-CA", "en-AU"];
...
export function getAllowedLocalesForCountry(country) {
  void country;
  return launchFormattingLocales;
}

export function getDefaultLocaleForCountry(country) {
  const market = getCountryRegionalConfig(country);
  const defaultLocale = normalizeString(market?.defaultLocale);

  if (launchFormattingLocales.includes(defaultLocale)) {
...
export function resolveLaunchFormattingLocale(locale = "", country = "") {
  const normalizedLocale = normalizeLocale(locale);

  if (launchFormattingLocales.includes(normalizedLocale)) {
  ...
  if (normalizedCountry) {
    return getDefaultLocaleForCountry(normalizedCountry);
  }

  return fallbackRegionalMarket.defaultLocale;
}
...
export function getAllowedCurrenciesForCountry(country) {
  return getCountryRegionalConfig(country)?.allowedCurrencies || [];
}
...
export function validateRegionalSelection({
  ...
  const normalizedLocale = normalizeLocale(locale);
  if (normalizedLocale && !launchFormattingLocales.includes(normalizedLocale)) {
...
export function resolveRegionalDefaults({
  ...
  return {
    ...
    locale: launchFormattingLocales.includes(normalizedLocale) ? normalizedLocale : getDefaultLocaleForCountry(market.country),
